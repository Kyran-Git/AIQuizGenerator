...
class AnalyzeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: questionsByTopic.isEmpty
          ? _buildEmptyState()
          : ListView.builder(
              padding: const EdgeInsets.all(16),
              itemCount: questionsByTopic.keys.length,
              itemBuilder: (context, index) {
                final topic = questionsByTopic.keys.elementAt(index);
                final questions = questionsByTopic[topic]!;
                return _TopicCard(topic: topic, questions: questions);
              },
            ),
    );
  }
...
class _TopicCard extends StatelessWidget {
  ...
  const _TopicCard({required this.topic, required this.questions});
